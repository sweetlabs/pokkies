var Instapaper = function() {
    ...
    this._hide_login = function(e, cancel) {
        if(cancel) {
            setTimeout(function() {
                self.feedback.set_message('You must log in to save to Instapaper');
                self.feedback.show();
            }, 500);
        }
...
Instapaper.prototype._init_oauth = function() {
    ...
    var self = this;
    document.getElementById('login_form').addEventListener('submit', function(e) { self.handle_login(e); });
...
Instapaper.prototype.add_bookmark = function(data, callback) {
    ...
    if(!this.verify_login()) {
        this.login_callback = function(){
            self.add_bookmark(data, callback);
        ...
        this.auth.api_call('bookmarks/add', data, true, false, true, 
            function(data) {
                self.feedback.set_message('<strong>Bookmarked!</strong>');
                self.feedback.show();
